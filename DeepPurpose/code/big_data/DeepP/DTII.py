...
from DeepP.utils import *
from DeepP.model_helper import Encoder_MultipleLayers, Embeddings        
from DeepP.encoders import *

from torch.utils.tensorboard import SummaryWriter

drug_dict = {}
protein_dict = {}
with open("/home/yqyang/yls/DeepPurpose_DeepPurpose_outtest_2/dataset/drug_smiles.txt") as f:
    f1 = f.readlines()
for line in f1:
    drug_dict[line.split(",")[0]] = line.split(",")[1].strip()
del f1
with open("/home/yqyang/yls/DeepPurpose_DeepPurpose_outtest_2/dataset/protein_fasta.txt") as f:
    f1 = f.readlines()
for line in f1:
...
class DBTA:
	...
	def test_(self, data_generator, model, repurposing_mode = False, test = False):
		...
		for i, (v_d, v_p, label) in enumerate(data_generator):
			...
			label_ids = torch.from_numpy(np.array(label)).float().to('cpu').numpy()
			#print(label_ids)
		...
		if self.binary:
			...
			return roc_auc_score(y_label, y_pred), average_precision_score(y_label, y_pred), f1_score(y_label, outputs), log_loss(y_label, outputs), y_pred, y_label, y_pred
		else:
			if repurposing_mode:
				return y_pred
	...
	def train(self, num, val = None, test = None, verbose = True):
		...
		for epo in range(train_epoch):
			with open("/home/yqyang/yls/DeepPurpose_DeepPurpose_outtest_2/dataset/fold/fold_" + str(num) + "/train.txt") as f:
				f1 = f.readlines()
			...
			if verbose:
				if (int(zzz) % 1000 == 0):
					t_now = time()
					print('Training at Epoch ' + str(epo + 1) + ' iteration ' + str(zzz) + \
			...
			if val is not None:
				...
				with torch.set_grad_enabled(False):
					if self.binary:  
						...
						auc, auprc, f1, loss_val, logits, y_lab, y_pre = self.test_(validation_generator, self.model)
			...
			if test is not None:
				...
				if self.binary:
					...
					auc, auprc, f1, loss_test, logits, y_l, y_p = self.test_(testing_generator, self.model, test = True)
					test_table = PrettyTable(["epoch", "AUROC", "AUPRC", "F1"])
					test_table.add_row(list(map(float2str, [(epo + 1), auc, auprc, f1])))
					if verbose:
						print('Validation at Epoch '+ str(epo + 1) + ' , AUROC: ' + str(auc)[:7] + \
						  ' , AUPRC: ' + str(auprc)[:7] + ' , F1: '+str(f1)[:7] + ' , Cross-entropy Loss: ' + \
						  str(loss_test)[:7])				
				...
				np.save(os.path.join(self.result_folder, str(self.drug_encoding) + '_' + str(self.target_encoding) 
					     + "_" + str(epo + 1) + '_logits.npy'), np.array(logits))                
		
				######### learning record ###########

				### 1. test results
				# prettytable_file = os.path.join(self.result_folder, "test_markdowntable" + "_" + str(epo + 1) +".txt")
				# with open(prettytable_file, 'w') as fp:
				# 	fp.write(test_table.get_string())

			output_loss = output_loss + str(loss_test)[:7] + "\n"
		...
		if test is not None:
			#### after training 
			prettytable_file = os.path.join(self.result_folder, "valid_each_epoch_markdowntable.txt")
			with open(prettytable_file, 'w') as fp:
				fp.write(test_table.get_string())
		...
		output_label_pred = "y_label,y_pred\n"
		if test is not None:
			if verbose:
				print('--- Go for Testing ---')
			if self.binary:
				auc, auprc, f1, loss, logits, y_label, y_pred = self.test_(testing_generator, model_max, test = True)
				for i in range(len(y_label)):
					output_label_pred = output_label_pred + str(y_label[i]) + "," + str(y_pred[i]) + "\n"
				pre_file = os.path.join(self.result_folder, "y_label_pred.txt")
				with open(pre_file, 'w') as fp:
					fp.write(output_label_pred)
