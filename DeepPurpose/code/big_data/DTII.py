...
drug_dict = {}
protein_dict = {}
with open("/home/databank/yls/DeepPurpose_DeepPurpose_outtest/dataset/drug_smiles.txt") as f:
    f1 = f.readlines()
for line in f1:
    drug_dict[line.split(",")[0]] = line.split(",")[1].strip()
del f1
with open("/home/databank/yls/DeepPurpose_DeepPurpose_outtest/dataset/protein_fasta.txt") as f:
    f1 = f.readlines()
for line in f1:
...
class DBTA:
	...
	def train(self, num, val = None, test = None, verbose = True):
		...
		for epo in range(train_epoch):
			with open("/home/databank/yls/DeepPurpose_DeepPurpose_outtest_1/dataset/fold/fold_" + str(num) + "/train.txt") as f:
				f1 = f.readlines()
			for m in range(len(f1)//64 + 1):
				smiles = []
				fasta = []
				label = []
				if m == len(f1)//64 + 1:
					for aaa in f1[-64: -1]:
						...
						label.append(aaa.split(",")[2])
				else:
					for aaa in f1[m*64: (m+1)*64]:
						smiles.append(drug_dict[aaa.split(",")[0]])
						fasta.append(protein_dict[aaa.split(",")[1]])
						label.append(aaa.split(",")[2])
				...
				if verbose:
					if (m % 1000 == 0):
						t_now = time()
						print('Training at Epoch ' + str(epo + 1) + ' iteration ' + str(m) + \
