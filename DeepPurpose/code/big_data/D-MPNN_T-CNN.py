...
# import pickle
from DeepP import utils
from DeepP.utils import *
from DeepP import DTII as models
...
def sampling(DTI):
    ...
    for key in dti_mutidict:
        ...
        for a in target_no:
            ...
            whole_negetive.append([key,a,0])
    print("0 and 1 finshed!")
    ...
    np.random.seed(10)
    whole_negetive_index = np.random.choice(np.arange(len(whole_negetive)), size=len(whole_positive),replace=False)
    ...
    data_set = np.vstack((whole_positive,whole_negetive_1))
    #print(len(data_set),data_set[0])
    print("all dataset finshed!")
    ...
    # print(output)
    print(np.array(fold_dataset).shape)
    print(fold_dataset[0][0][0])
    return fold_dataset
...
def val_encode(num):
    ...
    fas_val = []
    la_val = []
    with open("/home/yqyang/yls/DeepPurpose_DeepPurpose_outtest_2/dataset/fold/fold_" + str(num) + "/val.txt") as f:
        f1 = f.readlines()
    for line in f1:
...
def o_test(num):
    ...
    fas_val = []
    la_val = []
    with open("/home/yqyang/yls/DeepPurpose_DeepPurpose_outtest_2/out_test/test_" + str(num) + "/test.txt") as f:
        f1 = f.readlines()
    for line in f1:
        smi_val.append(models.drug_dict[line.split(",")[0]])
        fas_val.append(models.protein_dict[line.split(",")[1]])
...
def out_test():
    out_dict_drug_encoding = smiles_embed("./out_test/drug_smiles.txt")
    out_dict_protein_encoding = protein_embed("./out_test/protein_fasta.txt")
    target_set = list(out_dict_protein_encoding.keys())
    with open("./out_test/drug_protein.txt") as f:
        f1 = f.readlines()
    dti_mutidict = defaultdict(list)
    [dti_mutidict[i.split(",")[0]].append(i.split(",")[1].strip()) for i in f1]
    whole_positive = []
    ...
    for key in dti_mutidict:
        for i in dti_mutidict[key]:
            whole_positive.append([key,i,out_dict_drug_encoding[key],out_dict_protein_encoding[i],1])
        target_no = list(target_set)[:]
        [target_no.remove(i) for i in dti_mutidict[key]]
        for a in target_no:
            whole_negetive.append([key,a,out_dict_drug_encoding[key],out_dict_protein_encoding[a],0])
    whole_positive = np.array(whole_positive,dtype=object)
    whole_negetive = np.array(whole_negetive,dtype=object)
    data_set = np.vstack((whole_positive,whole_negetive))
    ou_test = pd.DataFrame(data_set)
    ou_test.rename(columns={0:'ICV',
                        1: 'Target',
                        2: 'drug_encoding',
                        3: 'target_encoding',
                        4: 'Label'}, 
